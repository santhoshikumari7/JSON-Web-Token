...
function Signup() {
    ...
    let onSubmitUsingJSON = async ()=>{
       ...
       let JSONData = await fetch("http://localhost:5000/signup",reqOptions);
    ...
    <img className="profilePicPreview"
     src={profilePicPath} alt=''></img>
       ...
       <button type="button"
        ...
        >onSubmit Using URLEncoded</button>
        <br></br>
        <button type="button" onClick={()=>{
           onSubmitUsingFormData();
        }}
        >onSubmit Using FormData</button>
      </div>
